$(document).ready(function () {
    ...
    $(document).on('change', '.field-value input[type="checkbox"]', function () {
        ...
        if (field === 'status' && value) {
            ...
            if (!ulsaIdValue || ulsaIdValue === 'Sin asignar') {
                ...
                alert(
                    'No se puede activar el candidato. Debe tener una Clave ULSA asignada primero.'
                );
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (res.success) {
                    ...
                    fieldText.text(
                        !value ? (field === 'status' ? 'Activo' : 'Aprobado') : 'Pendiente'
                    );
    ...
    function updateStatusCheckboxState(candidateId) {
        ...
        const ulsaIdField = candidateDetails.find('[data-field="ulsa_id"] .field-text');
        const statusCheckbox = candidateDetails.find(
            '[data-field="status"] input[type="checkbox"]'
        );
