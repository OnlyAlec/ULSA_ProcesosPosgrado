        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (res.success) {
                    ...
                    } else if (form.find('input[name="action"]').val() === 'deleteOneStudent') {
                        // Asumiendo que el action para eliminar único es deleteOneStudent basado en GA
                        form[0].reset();
                    }
                } else {
    ...
    function loadCandidatesTable() {
        ...
        $.ajax({
            ...
            beforeSend: function () {
                tableBody
                    .empty()
                    .html('<tr><td colspan="5" class="text-center">Cargando...</td></tr>'); // Mostrar mensaje de carga
            },
            success: function (response) {
                let res;
                try {
    ...
    function setupDatalist(inputSelector, optionsSelector) {
        ...
        $(document).on('click', function (event) {
            if (
                ...
                !$(event.target).closest(optionsSelector).length
            ) {
                optionsList.hide();
            }
        });
